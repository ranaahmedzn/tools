...
- **Dynamic Extensions:** The script detects the original extension (e.g., `.webp`, `.jpg`, `.png`) and preserves it during renaming.
- **Configurable Settings:** Easily customize the prefix, the starting number, and the target directory.
...
1. Navigate to the `image-renamer` directory in your terminal:
   ...
   ```
2. No external dependencies are needed! The tool only relies on built-in Node.js modules (`fs` and `path`).
...
const CONFIG = {
  ...
  prefix: 'client',                                   // Prefix for the new file names
